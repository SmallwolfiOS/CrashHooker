...
@implementation CrashHooker

+ (void)enbleCrashHookMode{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        [NSObject crashHookExchangeMethod];
        
        [NSArray crashHookExchangeMethod];
        [NSMutableArray crashHookExchangeMethod];
        
        [NSDictionary crashHookExchangeMethod];
        [NSMutableDictionary crashHookExchangeMethod];
        
        [NSString crashHookExchangeMethod];
        [NSMutableString crashHookExchangeMethod];
        
        [NSAttributedString crashHookExchangeMethod];
...
+ (void)noteErrorWithException:(NSException *)exception defaultToDo:(NSString *)defaultToDo {
    ...
    NSString *mainCallStackSymbolMsg = [CrashHooker getMainCallStackSymbolMessageWithCallStackSymbols:callStackSymbolsArr];
    ...
    //errorReason 可能为 -[__NSCFConstantString CrashHookCharacterAtIndex:]: Range or index out of bounds
    //将CrashHook去掉
    errorReason = [errorReason stringByReplacingOccurrencesOfString:@"CrashHook" withString:@""];
    
    NSString *errorPlace = [NSString stringWithFormat:@"Error Place:%@",mainCallStackSymbolMsg];
    
    NSString *logErrorMessage = [NSString stringWithFormat:@"\n\n%@\n\n%@\n%@\n%@\n%@",CrashHookSeparatorWithFlag, errorName, errorReason, errorPlace, defaultToDo];
    
    logErrorMessage = [NSString stringWithFormat:@"%@\n\n%@\n\n",logErrorMessage,CrashHookSeparator];
    CrashHookLog(@"%@",logErrorMessage);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:CrashHookNotification object:nil userInfo:errorInfoDic];
    });
}
@end
